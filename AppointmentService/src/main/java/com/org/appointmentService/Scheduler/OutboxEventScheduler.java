	...
	public void publishPendingEvents() {
		...
		for (OutboxEvent outboxEvent : outboxEventList) {
			try {
				...
				// continue; parked events are replayed only by an explicit
				// admin replay, since replaying them after newer events for
				// the same entity can regress projections.
				if (isFatal(ex) || outboxEvent.getRetryCount() >= MAX_RETRIES) {
					outboxEvent.setParked(true);
					outboxEventRepository.save(outboxEvent);
					logger.warn("Parked poison outbox event {} (type={}, retries={}) — admin replay required",
